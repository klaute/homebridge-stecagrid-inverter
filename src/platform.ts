...
import { StecaGridObject, Site } from './types/type';
    ...
    setInterval(() => {
      if (this.pauseToTime<new Date() && this.callingService===false) {
        ...
        httpRequest.GetStatusListForAll().then((results)=> {
          ...
          const stecagridObject = (<StecaGridObject>results);

          if (stecagridObject !== undefined && stecagridObject.Body !== undefined &&
          stecagridObject.Body.Data!==undefined && stecagridObject.Body.Data.Site!==undefined) {

            const site = stecagridObject.Body.Data.Site;
